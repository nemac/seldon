...
function layerPicker(activeMapView){
    ...
    //The draggable causes script issues in IE8
    //for now I will not utilize it... better yet
    //Only chrome and firefox get this feature....
...
$(".imgDialog").live("click",function(e){
	//check out orig element > src element > outer of html on the object e, and key off of name...
	$("#opacityDialog").dialog({ zIndex:10050, position:"left", hide:"explode" });
...
function initOpenLayers() {
    ...
    map.addLayers([baseMapLayers[0].OLBaseMapLayer,activeOLWMSLayers[1].OLWMSLayer,activeOLWMSLayers[30].OLWMSLayer]);
    ...
    $("#legend_accordion").append('<div id="lgd'+activeOLWMSLayers[30].lid+'" class="lgd'+activeOLWMSLayers[30].lid+'"><img src="'+activeOLWMSLayers[30].legend+'"/></div>');     
    ...
    $("#lgd"+activeOLWMSLayers[30].lid+"").click(function() {
        //remove legend graphic, layerpicker checkedbox and map layer
        $('div').remove('.lgd'+activeOLWMSLayers[30].lid+''); //remove legend graphic
        //filteredOLWMSLayer = activeOLWMSLayers.filter(function(WMSLayer){return (WMSLayer.lid==activeOLWMSLayers[1].lid);});
        filteredOLWMSLayer = filterObjectArrayByVal(activeOLWMSLayers,"lid",activeOLWMSLayers[30].lid);
		map.removeLayer(filteredOLWMSLayer[0].OLWMSLayer); //remove map layer
        $('input:checkbox[id="chk'+filteredOLWMSLayer[0].lid+'"]').attr('checked',false);
    });  
