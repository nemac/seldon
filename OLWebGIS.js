...
function parseConfig(document){
    ...
    if (!sharedMapURL)
    {
        ...
        layerPicker(mapViews[1], 3);  //pass the default theme as the current theme
    }
...
function createLayerToggleCheckbox(lid, checked) {
    ...
    return $('<input type="checkbox" id="chk'+lid+'" ' + checkedattr + '></input>').click(function(){
        ...
        if($(this).is(':checked')){ //layerPicker checked
            ...
            addLayerToLegend(filteredOLWMSLayer[0].lid, filteredOLWMSLayer[0].legend);
...
function addLayerToLegend(lid, imageurl) {
...
function layerPicker(activeMapView, openToAccordGrp){
    ...
    // $("#layerPickerAccordion").append('<h3><b><a href="#layersAccordion">Layer Picker:</a></b></h3>');
    // $("#layerPickerAccordion").append('<div class="layers-header">Handle here to drag!</div>');
    $('#layerPickerAccordion').listAccordion();
/*
    g = $('#layerPickerAccordion').listAccordion('addSection', '<a href="#mapToolsAccordion">Layer Picker:</a>');
    s = $('#layerPickerAccordion').listAccordion('addSublist', g, '<div class="layers-header">Handle here to drag!</div>');
*/
    ...
    $(activeMapViewViewGroups).each(function(index) {
        ...
        if (activeMapViewViewGroups[index].nodeType==1) {
            ...
            g = $('#layerPickerAccordion').listAccordion('addSection', '<a href=#Accordion'+accordianNum+' gid='+activeWMSGroup[0].gid+'>'+activeWMSGroup[0].label+'</a>');
            ...
            //$("#layerPickerAccordion").append('<div>'+accordString+'</div>');
...
$(".layerPropertiesIcon").live("click",function(e){
...
function initOpenLayers() {
    ...
    if (!sharedMapURL) { //Begin default layers prep------------>
        ...
        //States
        addLayerToLegend(activeOLWMSLayers[1].lid, activeOLWMSLayers[1].legend);

        //Current NRT product
        addLayerToLegend(activeOLWMSLayers[33].lid, activeOLWMSLayers[33].legend);
